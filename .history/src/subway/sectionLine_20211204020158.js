...
export default class SectionLine {
            ...
            stations
              ? stations.map((station, idx) => {
                ...
                <tr>
                  ...
                  <td>${station}</td>
                  <td>
                    <button class="section-delete-button" data-line-name=${target.dataset.lineName}>노선에서 제거</button>
                  </td>
                </tr>
              `;
                })
  ...
  static setSubmitSectionEvent = ({ target }) => {
    if (target.id === "section-add-button") {
      ...
      console.log(target.dataset.lineName);
      const lineManager = getLocalStorage("line-manager").find(
        (line) => Object.keys(line)[0] === target.dataset.lineName
      )[target.dataset.lineName];
      const stationName =

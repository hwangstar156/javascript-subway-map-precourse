import { isDuplicatedName } from "../util/check-valid.js";
  ...
  static setDeleteLineNameEvent = ({ target }) => {
    ...
    setLocalStorage(
      ...
      lineManagerList.filter(
        (lineManager) => lineManager.lineName !== lineNameId
      )
    );
    this.renderLineManager();
  };

  static createAlertMessage = (nameLists, newName) => {
    if (isDuplicatedName(nameLists, newName)) {
      return "호선이 중복되었습니다";
    }

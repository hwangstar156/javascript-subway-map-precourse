...
export default class SectionLineView {
  renderSectionLineInput = (lineManager) => {
    const template = this.createSectionLineInputTemplate(lineManager);
    resetInput();
    showInput(template);
  };
  ...
  renderSectionLineEdit = (stationNames, stations, lineName) => {
    const template = this.createSectionLineEditTemplate(
      stationNames,
      stations,
      lineName
    );
  ...
  static createSectionLineEditTemplate = (stationNames, stations, lineName) => {
    return `
        <h2>${lineName} 관리</h2>
        <h3>구간 등록</h3>
